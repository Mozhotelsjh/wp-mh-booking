...
class OW_Workflow_Service {
    ...
    * @since 2.0
    */
   public function get_workflow_from_result_set( $result ) {
      if ( ! $result ) {
         return "";
      }
    ...
    * @since 2.0
    */
   public function save_step_as_new( $new_wf_id, $current_step_id ) {
      global $wpdb;
   ...
   public function get_post_count_in_workflow( $wf_id = false ) {
      ...
      $sql = "SELECT WS.workflow_id, COUNT(*) post_count, GROUP_CONCAT(post_id) post_ids  FROM " . OW_Utility::instance()->get_action_history_table_name() . " AH
   ...
   /**
    * Get the first step in the workflow - ajax function
    *
    ...
    * @since 2.0
    */
   public function get_workflow_list1( $action = null ) {
      global $wpdb;

      $workflows = array();
      $current_time = date( "Y-m-d" );
      if ( $action == "all" || empty( $action ) ) {
         $sql = "SELECT * FROM " . OW_Utility::instance()->get_workflows_table_name() . " ORDER BY name desc";
      }
      ...
      $workflow_result_set = apply_filters( 'owf_filter_workflow_results', $workflow_result_set );
...
// If end date is not provided then workflow will be valid
      ...
      if ( ! empty( $additional_info[ 'wf_for_roles' ] ) ) {
         ...
         if ( ! in_array( $current_role, $additional_info[ 'wf_for_roles' ] ) ) {
            return false;
         }
      }

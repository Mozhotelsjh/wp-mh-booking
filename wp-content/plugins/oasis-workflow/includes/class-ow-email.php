...
class OW_Email {
   ...
   public function get_step_comment_content( $action_id ) {
      ...
      foreach ( $comments as $comment ) {
         ...
         if( get_option( 'oasiswf_priority_setting' ) == 'enable_priority' ){
            $priority_value = get_post_meta( $action_step->post_id, 'ow_task_priority', TRUE );
         ...
         if ( ! empty( $due_date )) {
            ...
            $due_date_text = $workflow_terminology_options['dueDateText'];
   ...
   public function post_published_notification( $new_status, $old_status, $post ) {
      ...
      $is_meta_exist = get_post_meta( $post->ID, 'oasis_is_in_workflow' );

      if ( $email_settings['post_publish_emails'] == "yes"
           && ! empty( $is_meta_exist )
           && ( $old_status == 'publish' || $old_status == "future" ) // temporary fix, since the old and new status of publish happens only once, so email is sent only once.
           && $new_status == 'publish'
           ...
           && $post->post_status != 'inherit')
      {
         ...
         $msg = sprintf( __( '<div>Hello %1$s,</div><p>Your article <a href="%2$s" title="%3$s">%3$s</a> has been published on <a href="%4$s" title="%5$s">%5$s</a></p><p>Thanks.</p>', 'oasisworkflow' ), $user->display_name, esc_url( $post->guid ), $post->post_title, esc_url(get_bloginfo('url')),  get_bloginfo('name'));
   ...
   public function send_abort_email_to_author( $post_id ) {
      ...
      if ( isset($email_settings['abort_email_to_author']) && $email_settings['abort_email_to_author'] == 'yes' ) {
         ...
         $message .= "<a href=" . esc_url( $post->guid ) . " title=" . $post->post_title . ">" . $post->post_title . "</a>";
   ...
   public function notify_users_on_unauthorized_update( $assignees, $current_user_id, $post_id ) {
      ...
      if ( $email_settings['unauthorized_post_update_emails'] == "yes" )
      {
         ...
         foreach ($assignees as $assignee) {
            ...
            $msg = $msg . "<a href=" . esc_url( $post_info->guid ) . " title=" . $post_info->post_title . ">" . $post_info->post_title . "</a>";
   ...
   */
   ...
   public function post_submit_notification( $post_id, $new_action_history_id ) {
      ...
      if ( isset( $email_settings['submit_to_workflow_email'] ) && $email_settings['submit_to_workflow_email'] == "yes" ) {
         ...
         $msg = sprintf( __( '<div>Hello %1$s,</div><p>Your article <a href="%2$s" title="%3$s">%3$s</a> has been successfully submitted to the workflow.</p><p>Thanks.</p>', 'oasisworkflow' ), $user->display_name, esc_url( $post->guid ), $post->post_title, esc_url(get_bloginfo('url')),  get_bloginfo('name'));
         $message = '<html><head></head><body><div class="email_notification_body">'.$msg.'</div></body></html>';

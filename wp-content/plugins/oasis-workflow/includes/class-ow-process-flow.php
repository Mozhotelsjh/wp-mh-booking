...
class OW_Process_Flow {
   ...
   public function __construct() {
      ...
      add_action( 'wp_ajax_get_users_in_step', array( $this, 'get_users_in_step' ) );
      add_action( 'wp_ajax_get_pre_next_steps', array( $this, 'get_pre_next_steps' ) );
      add_action( 'wp_ajax_get_post_status_by_history_id', array( $this, 'get_post_status_by_history_id' ) );
      add_action( 'wp_ajax_change_workflow_status_to_complete', array( $this, 'change_workflow_status_to_complete' ) );
      add_action( 'wp_ajax_change_workflow_status_to_cancelled', array( $this, 'change_workflow_status_to_cancelled' ) );
      add_action( 'wp_ajax_submit_post_to_step', array( $this, 'submit_post_to_step' ) );
      add_action( 'save_post', array( $this, 'update_oasis_is_in_workflow' ), 10, 1 );
      add_action( 'wp_ajax_exit_post_from_workflow', array( $this, 'exit_post_from_workflow' ) );
      add_action( 'wp_ajax_multi_abort_from_workflow', array( $this, 'multi_abort_from_workflow' ) );
      add_action( 'wp_ajax_get_post_publish_date_edit_format', array( $this, 'get_post_publish_date_edit_format' ) );
      add_action( 'wp_ajax_is_original_post', array( $this, 'is_original_post' ) );
      ...
      add_action( 'wp_ajax_submit_post_to_workflow_pre', array( $this, 'submit_post_to_workflow_pre' ) );
      add_action( 'wp_ajax_sign_off_post_from_workflow_pre', array( $this, 'sign_off_post_from_workflow_pre' ) );

      // Show posts which current logged in user can access or see..
//      add_action( 'pre_get_posts', array( $this, 'show_only_accessible_posts' ) );

      // team actions
      add_action( 'wp_ajax_has_users_in_team', array( $this, 'has_users_in_team' ) );
      add_action( 'wp_ajax_get_team_in_workflow', array( $this, 'get_team_in_workflow' ) );
   ...
   public function get_task_count_by_priority( $user_id ) {
      ...
      $sql = "SELECT postmeta.meta_value AS priority,
                ...
                JOIN {$wpdb->postmeta} AS postmeta
                  ...
                  AND postmeta.meta_key = 'ow_task_priority'
   ...
   public function reassign_process() {
      ...
      $ow_email = new OW_Email( );
      if( $action_history_id && $reassign_users ) {
         $ow_history_service = new OW_History_Service();
         ...
         if( $data["assign_actor_id"] != -1 ) { // assignment or publish step (reassigned)
            ...
            foreach ( $reviews as $review ) {
               if( in_array( $review->actor_id, $reassign_users ) ) {
                  echo sprintf( __( "User '%s' has already been assigned this task. Please select another user.", "oasisworkflow" ), OW_Utility::instance()->get_user_role_and_name( $review->actor_id )->username );
                  exit();
               }
            }
   ...
   public function get_sign_off_date( $action_history_row ) {
      ...
      if( $action_history_row->action_status == "claim_cancel" ) {
         $ow_history_service = new OW_History_Service();
         $claimed_row = $ow_history_service->get_action_history_by_parameters( "claimed", $action_history_row->step_id,
            $action_history_row->post_id, $action_history_row->from_id );
   ...
   /**
    * Internal function - to get users in step
    * @param int $step_id
    * @param int|null $post_id
    * @return mixed users and processes in the step
    */
   public function get_users_in_step_internal( $step_id, $post_id = null ) {
      if( $step_id == "nodefine" ) {
         return null;
      }
      ...
      if( $wf_info ) {
         ...
         if( $task_users ) {
            $result["users"] = $task_users;
            $result["process"] = $step_info->process;
            $result["assign_to_all"] = isset( $step_info->assign_to_all ) ? $step_info->assign_to_all : 0;
         }
      }
      return $result;
   }
   ...
   /**
    * Get users in step - ajax function
    *
    * @return mixed users and processes in the step
    */
   public function get_users_in_step() {
      check_ajax_referer( 'owf_signoff_ajax_nonce', 'security' );

      $step_id = intval( sanitize_text_field( $_POST["stepid"] ) );
      $post_id = isset( $_POST["postid"] ) ? intval( $_POST["postid"] ) : null;

      $users_and_processes = $this->get_users_in_step_internal( $step_id, $post_id );
      if( $users_and_processes != null ) {
         echo json_encode( $users_and_processes );
         // no users defined
      } else {
         echo "false";
      }
   ...
   /**
    * Internal function to get the next or previous steps
    * @param int $action_history_id
    * @return mixed step information
    *
    * @since 2.0
    */
   public function get_pre_next_steps_internal( $action_history_id ) {
      $action_history_id = intval( $action_history_id );
   ...
   /**
    * function to get the next or previous steps - ajax function
    * @param int $action_history_id
    * @return mixed step information
    *
    * @since 2.0
    */
   public function get_pre_next_steps() {
      check_ajax_referer( 'owf_signoff_ajax_nonce', 'security' );

      $action_history_id = intval( sanitize_text_field( $_POST["oasiswfId"] ) );
      $steps = $this->get_pre_next_steps_internal( $action_history_id );

      wp_send_json_success( $steps );
   }

   /**
    * When the workflow progresses to the next/previous (ie. success/failure states),
    * We need to update the post status with the status set in the workflow_info connset
    * 
    * @param int $post_id
    * @param int $from_step_id - current(source) step of workflow
    * @param string $immediately - do you want to publish the article immediately or at a set time.
    * @param bool $new_action_history_id - the newly added history id
    * 
    * @since 2.1
    */
   public function copy_step_status_to_post( $post_id, $from_step_id, $immediately = null, $new_action_history_id ) {
      global $wpdb;

      $from_step_id = intval( $from_step_id );
      $post_id      = intval( $post_id );
      if( ! empty( $immediately ) ) { // scheduled post
      ...
      if( $immediately ) { // user intends to schedule the post
         ...
         if( $step_status == "publish" && $im_dt > $now_dt ) { // for new posts
         ...
         remove_action( 'save_post', array( $this, 'update_oasis_is_in_workflow' ), 10 );
    ...
    */
   public function update_oasis_is_in_workflow( $post_id ) {
      global $wpdb;

      $post_id = intval( $post_id );
      $is_in_workflow = get_post_meta( $post_id, 'oasis_is_in_workflow', true );
...
//						jQuery('#post-status-display').parent().hide() ;
   ...
   public function cleanup_after_workflow_complete( $post_id ) {
      ...
      delete_post_meta( $post_id, "oasis_is_in_team" );
   }
   ...
   /**
    * A pre hook before submit to workflow. Calls - owf_submit_to_workflow_pre hook.
    *
    * Can be used by add-ons or plugins to insert any functionality before submit to workflow.
    *
    * @return $result - could be an array OR "true"
    * @since 2.0
    */
   public function submit_post_to_workflow_pre() {
      ...
      if( !empty( $_POST['tax_input']['post_tag'] ) ) {
         $post_tags = count( explode( ',', sanitize_text_field( $_POST['tax_input']['post_tag'] ) ) );
      }
      // for some reason, there is an entry for "0" in the list, so lets minus that
      $post_category = (isset( $_POST['post_category'] )) ? intval( count( $_POST['post_category'] ) ) - 1 : 0;

      // TODO: there is a function to check $_POST['index'] is array or not in wordpress
      ...
      $post_data['post_tag'] = $post_tags;
      $post_data['category'] = $post_category;

      $args = array( $post_id, $step_id, $result, $post_data );

      // let the hook run some pre validations and return validation error messages
      do_action_ref_array( 'owf_submit_to_workflow_pre', array( &$args ) );
      $result = $args[2];

      if( !empty( $result ) ) {
         echo $result;
      } else {
         echo "true"; // echo true if there are no results to be displayed
      }
   ...
   /**
    * A pre hook before submit to workflow. Calls - owf_sign_off_from_workflow_pre hook.
    *
    * Can be used by add-ons or plugins to insert any functionality before submit to workflow.
    *
    * @return $result - could be an array OR "true"
    * @since 2.0
    */
   public function sign_off_post_from_workflow_pre() {
      ...
      // if form attr is set then its value from edit post else from inbox page
      if( isset( $_POST['form'] ) && ! empty( $_POST['form'] ) ) {
         ...
         if ( isset( $_POST['post_category'] ) ) {
            $post_category = intval( count( $_POST['post_category'] ) - 1 );
         }
         ...
         $data = @array_map( 'esc_attr', $_POST );
         $sanitize_excerpt = isset( $data['excerpt'] ) ? $data['excerpt'] : '';
         $post_contents = $_POST['content'];
      } else {
         $data = (array) get_post( $post_id );
         $post_contents = $data['post_content'];
         $sanitize_excerpt = isset( $data['post_excerpt'] ) ? $data['post_excerpt'] : '';
         $post_tags = count( wp_get_post_tags( $post_id ) );
         $post_category = count( wp_get_post_categories( $post_id ) );
      }
      ...
      $post_data = array();
      $post_data['post_title'] = $data['post_title'];
      $post_data['post_content'] = $post_contents;
      $post_data['post_excerpt'] = $sanitize_excerpt;
      $post_data['post_tag'] = $post_tags;
      $post_data['category'] = $post_category;

      $args = array( $post_id, $step_id, $result, $post_data, $decision );

      // let the hook run some pre validations and return validation error messages
      do_action_ref_array( 'owf_sign_off_from_workflow_pre', array( &$args ) );
      $result = $args[2];
   ...
   public function submit_post_to_workflow() {
      ...
      update_post_meta( $post_id, "ow_task_priority", $priority );

      $team_id = null;
      $actors = sanitize_text_field( $_POST["hi_actor_ids"] );
      $assign_to_all = isset( $_POST['assign_to_all'] ) ? intval( sanitize_text_field( $_POST['assign_to_all'] ) ) : 0;
      if( 1 === $assign_to_all ) {
         $users_and_processes = $this->get_users_in_step_internal( $step_id, $post_id );
      ...
      $actors_info = array( $actors, $step_id, $post_id, $team_id );
      ...
      $this->submit_post_to_workflow_internal( $step_id, $post_id, $actors, $due_date, $comments, $team_id );
   }

   /**
    * submit post to workflow - internal
    */
   public function submit_post_to_workflow_internal( $step_id, $post_id, $actors, $due_date, $user_comments, $team_id = null ) {
      ...
      $actors = sanitize_text_field( $actors );
      if( !empty( $team_id ) ) {
      ...
      $comments[] = array( "send_id" => $user_id, "comment" => stripcslashes( $user_comments ), "comment_timestamp" => current_time( "mysql" ) );
      ...
      $adata = array(
         ...
         'comment' => wp_json_encode( $acomments ),
         'step_id' => $step_id,
         'post_id' => $post_id,
         'from_id' => '0',
      ...
      $action_history_table = OW_Utility::instance()->get_action_history_table_name();
      $adata["assign_actor_id"] = $user_id;
      $new_action_history_id = OW_Utility::instance()->insert_to_table( $action_history_table, $adata );  // insert record in history table for workflow submit
      //-----------------------------------------
      $data = array(
      ...
      if( !empty( $due_date ) ) {
         $data["due_date"] = OW_Utility::instance()->format_date_for_db_wp_default( $due_date );
      }
      $new_action_history_id = $this->save_action( $data, $actors );
      update_post_meta( $post_id, "oasis_is_in_workflow", 1 ); // set the post meta to 1, specifying that the post is in a workflow.
      if( !empty( $team_id ) ) {
         update_post_meta( $post_id, "oasis_is_in_team", $team_id ); // set the post meta to the team_id, specifying that the post is assigned to a specific team
      }
   ...
   /*
    * Submit post to step - ajax function
    */
   public function submit_post_to_step() {
      ...
      // check for assign to all
      $assign_to_all = intval( $_POST['assign_to_all'] );
      if( 1 === $assign_to_all ) { // if assign to all is checked, assign the task to all users
         //$users = OW_Utility::instance()->get_users_by_role( $step_info->assignee, $post_id );
         $assigned_actors = array();
         $users_and_processes = $this->get_users_in_step_internal( $step_id, $post_id );
         $users = $users_and_processes["users"];
         foreach ( $users as $user ) {
            $assigned_actors[] = $user->ID;
         }
         $assigned_actors = implode( "@", $assigned_actors );
      } else { // else assign the task to selected users only
      ...
      // sanitize_text_field remove line-breaks so do not sanitize it.
      $sign_off_comments = nl2br( $_POST["hi_comment"] );
      $sign_off_status = sanitize_text_field( $_POST['review_result'] );
      ...
      $new_action_history_id = $this->submit_post_to_step_internal( $post_id, $history_id, $step_id, $task_user, $sign_off_comments, $assigned_actors, $sign_off_status, $due_date );

      wp_send_json_success( $new_action_history_id );
   }

   /**
    * submit post to step internal function
    */
   public function submit_post_to_step_internal( $post_id, $history_id, $step_id,
      $task_actor_id, $sign_off_comments, $assigned_actors, $sign_off_status, $due_date ) {
      global $wpdb;
      ...
      $team_id = get_post_meta( $history_details->post_id, 'oasis_is_in_team', true );
      ...
      if( $history_details->assign_actor_id == -1 ) { // the current step is a review step, so review decision check is required
         ...
         if( !empty( $due_date ) ) {
            $review_data["due_date"] = OW_Utility::instance()->format_date_for_db_wp_default( $due_date );
         }
         ...
         $new_action_history_id = $this->review_step_procedure( $history_id );
         ...
         //------post status change----------
         $this->copy_step_status_to_post( $post_id, $history_details->step_id, null, $new_action_history_id );
      }
    ...
    */
   public function review_step_procedure( $action_history_id ) {
    ...
    */
   public function review_step_everyone( $review_data, $action_history_id ) {
      /*
    ...
    */
   public function review_step_anyone( $review_data, $action_history_id ) {
    ...
    */
   public function change_review_status_to_no_action( $ddata, $action_history_id ) {
      global $wpdb;
    ...
    */
   public function save_review_action( $ddata, $action_history_id, $result ) {
      $ow_history_service = new OW_History_Service();
      $action = $ow_history_service->get_action_history_by_id( $action_history_id );
      ...
      //--------post status change---------------
      $this->copy_step_status_to_post( $action->post_id, $action->step_id, null, $new_action_history_id );
    ...
    */
   public function change_workflow_status_to_complete_internal( $post_id, $publish_datetime, $history_id, $current_actor_id ) {
      global $wpdb;

      $post_id = intval( sanitize_text_field( $post_id ) );
      $history_id = intval( sanitize_text_field( $history_id ) );
      $current_actor_id = intval( sanitize_text_field( $current_actor_id ) );
      if( $publish_datetime != null ) {
         $publish_datetime = sanitize_text_field( $publish_datetime );
      }

      if( $publish_datetime != null ) {
         $im_dt = new DateTime( $publish_datetime );
         $now_dt = new DateTime( current_time( 'mysql' ) );
      }
      ...
      $wpdb->update( $action_table, array( "review_status" => "complete",
          "update_datetime" => current_time( 'mysql' ) ), array( "action_history_id" => $history->ID ) );
      ...
      if( $new_action_history_id ) {
         ...
         if( ! empty( $original_post_id ) ) {
            ...
            // we will update the contents of the original post via the currentrev to draft hook
            if( $publish_datetime != null && $now_dt != $im_dt ) {
               $this->copy_step_status_to_post( $post_id, $action->step_id, $publish_datetime, $new_action_history_id );
            }
            $this->cleanup_after_workflow_complete( $post_id );
            $return_array = array (
               "post_status" => get_post_status( $post_id ),
               "workflow_status" => "complete_revision"
            );
            return $return_array;
            ...
            if( $publish_datetime != null && $now_dt != $im_dt ) {
               ...
               $this->copy_step_status_to_post( $post_id, $action->step_id, current_time( 'mysql' ), $new_action_history_id );
            }
            $this->cleanup_after_workflow_complete( $post_id );
            $return_array = array (
               "post_status" => get_post_status( $post_id ),
               "workflow_status" => "complete_original"
            );
            return $return_array;
         }
      }
   ...
   /*
    * Change status of workflow to complete - ajax function
    */
   public function change_workflow_status_to_complete() {
      ...
      wp_send_json_success( $post_complete_action );
   }

   /*
    * Cancel the workflow - ajax function
    */
   public function change_workflow_status_to_cancelled() {
      ...
      if( $new_action_history_id ) {
         ...
         $this->copy_step_status_to_post( $post_id, $action->step_id, null, $new_action_history_id );
         ...
         $full_name = OW_Utility::instance()->get_user_name( $current_userid );

         $msg = sprintf( __( '<div>Hello %1$s,</div><p>The post <a href="%2$s" title="%3$s">%3$s</a> has been cancelled from the workflow.</p>', 'oasisworkflow' ), $post_author->display_name, esc_url( $post->guid ), $post->post_title );
         ...
         //---------------------------------------------
         $this->cleanup_after_workflow_complete( $post_id );
         echo trim( $new_action_history_id );
      }
      die();
   }

   /*
    * See if the team has the users
    */
   public function has_users_in_team() {
      check_ajax_referer( 'owf_signoff_ajax_nonce', 'security' );
      $step_id = sanitize_text_field( $_POST["step_id"] );
      $post_ids = isset( $_POST["post_id"] ) ? $_POST["post_id"] : null;
      if( empty( $post_ids ) ) {
         $post_ids = isset( $_GET["post"] ) ? $_GET["post"] : null;
      }
      ...
      if( !empty( $post_ids ) ) {
         ...
         // sanitize the values
         $post_id_array = array_map( 'esc_attr', $post_id_array );
      ...
      $step = $ow_workflow_service->get_step_by_id( $step_id );
      $step_info = json_decode( $step->step_info );
      $assignee_roles = isset( $step_info->task_assignee->roles ) ? array_flip( $step_info->task_assignee->roles ) : $step_info->assignee;
   ...
   /*
    * get the team for the specific workflow
    */
   public function get_team_in_workflow() {
      check_ajax_referer( 'owf_signoff_ajax_nonce', 'security' );
      $team_id = "";
      $post_id = isset( $_POST["post_id"] ) ? intval( sanitize_text_field( $_POST["post_id"] ) ) : null;
      if( !empty( $post_id ) ) {
         $team_id = $this->get_team_in_workflow_internal( $post_id );
         if( $team_id != null ) {
   ...
   /*
    * get the team name for the post
    */
   public function get_team_in_workflow_internal( $post_id ) {
      $team_id = get_post_meta( $post_id, 'oasis_is_in_team', true );
      if( !empty( $team_id ) ) {
         return trim( $team_id );
      } else {
         return null;
   ...
   /**
    * Given the history_id and to step id, find the post status from the connection info - ajax
    *
    * @since 2.2
    */
   public function get_post_status_by_history_id() {
      check_ajax_referer( 'owf_signoff_ajax_nonce', 'security' );
      $old_action_history_id = intval( sanitize_text_field( $_POST["old_history_id"] ) );
      $target_id = sanitize_text_field( $_POST["to_step_id"] );

      // derive new post status from connection info
      $ow_workflow_service = new OW_Workflow_Service();
      $ow_history_service = new OW_History_Service();

      $history_details = $ow_history_service->get_action_history_by_id( $old_action_history_id );
      // get the source step id from the old history info
      $source_id = $history_details->step_id;
      ...
      wp_send_json_success( $step_status );
   }
    ...
    * abort workflow
    */
   public function exit_post_from_workflow() {
      global $wpdb;
      ...
      $this->abort_the_workflow( $history_id );
      die();
   }
   ...
   public function get_post_publish_date_edit_format() {
      $post_id = isset( $_POST["post_id"] ) ? intval( sanitize_text_field( $_POST["post_id"] ) ) : null;
      if( !empty( $post_id ) ) {
         $publish_date = get_the_date( OASISWF_EDIT_DATE_FORMAT . " @ H:i", $post_id );
         echo $publish_date;
      } else {
         echo "";
      }
    ...
    */
   public function get_immediately_content( $post_id, $is_future_date ) {
      if( $is_future_date ) {
   ...
   /**
    * Is the post original OR revision - ajax function
    *
    * @return boolean
    */
   public function is_original_post() {
      $history_id = intval( sanitize_text_field( $_POST['oasiswf_id'] ) );
      $ow_history_service = new OW_History_Service();
      $oasiswf = $ow_history_service->get_action_history_by_id( $history_id );
      $original_post_id = $this->get_original_post( $oasiswf->post_id );
   ...
   public function step_signoff_popup_setup() {
      ...
      if( get_option( "oasiswf_activate_workflow" ) == "active" &&
         ...
         } else if( current_user_can( 'ow_sign_off_step' ) && $chkResult == "not-assigned" &&
                 is_admin() && preg_match_all( '/page=oasiswf(.*)|post-new\.(.*)|post\.(.*)/', $_SERVER['REQUEST_URI'], $matches ) ) {
   ...
   public function when_post_trash_delete( $post_id ) {
      ...
      // when we trash the post, and it happens to be a revision, then only remmove "oasis_current_revision" from the original post
      ...
      delete_post_meta( $original_post, 'oasis_current_revision', $post_id );
   }
   
   /**
    * Check wheather team enabled
    * @return string
    */
   public function is_teams_available() {
      return get_option( 'oasiswf_team_enable', 'no' );
   }
   ...
   public function purge_user_assignments( $deleted_user_id ) {
      ...
      foreach ( $inbox_items as $inbox_item ) {
         ...
         $step_users = $this->get_users_in_step_internal( $step_id, $post_id );
         ...
         if( !$is_multi_user_task ) {
            ...
            $this->abort_the_workflow( $inbox_item->ID, false );
         }
      }
   }
   ...
   public function abort_the_workflow( $history_id, $print_id = true ) {
      ...
      if( $iid ) {
         ...
         $ow_email->send_abort_email_to_author( $action->post_id );
         if( $print_id ) {
            echo trim( $iid );
         }
      }
   ...
   public function owf_submit_to_workflow_hook_test( $post_id, $workflow_history_id ) {
      OW_Utility::instance()->logger( "this is testing the submit to workflow hook" );
      OW_Utility::instance()->logger( "PostID:" . $post_id );
      OW_Utility::instance()->logger( "Workflow History ID:" . $workflow_history_id );
   }

   public function owf_step_sign_off_hook_test( $post_id, $workflow_history_id ) {
      OW_Utility::instance()->logger( "this is testing the step sign off workflow hook" );
      OW_Utility::instance()->logger( "PostID:" . $post_id );
      OW_Utility::instance()->logger( "Workflow History ID:" . $workflow_history_id );
   }

   public function owf_workflow_complete_hook_test( $post_id, $workflowId ) {
      OW_Utility::instance()->logger( "this is testing the workflow completion hook" );
      OW_Utility::instance()->logger( "PostID:" . $post_id );
      OW_Utility::instance()->logger( "Workflow ID:" . $workflowId );
   }

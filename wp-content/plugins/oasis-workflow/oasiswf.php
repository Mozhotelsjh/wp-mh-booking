...
define( 'OASISWF_VERSION', '2.2' );
define( 'OASISWF_DB_VERSION', '2.2' );
...
class OW_Plugin_Init {
	...
	public function __construct() {
      ...
      /**
       * Display Workflow dashboard widget
       * @since 2.1
       */
   ...
   public function load_css_and_js_files() {
	...
	public function load_all_classes() {
      ...
      include_once( OASISWF_PATH . 'includes/class-ow-inbox-service.php' );
		include_once( OASISWF_PATH . 'includes/class-ow-license-service.php' );

		/**
		 * Settings classes
		 */
		include_once( OASISWF_PATH . 'includes/class-ow-license-settings.php' );
	...
	private function run_for_site() {
      ...
      foreach( $custom_statuses as $custom_status => $desc ) {
		...
		if ( ! get_option( 'oasiswf_review_process_setting' ) ) {
			update_option( "oasiswf_review_process_setting", $review_process_setting );
		}
	...
	private static function delete_for_site() {
		...
		delete_post_meta_by_key( 'oasis_is_in_workflow' );
	...
	private function install_admin_data() {
		...
		// step 1 - review
		$review_step_info = '{"process":"review","step_name":"review","assign_to_all":0,"task_assignee":{"roles":["editor"],"users":[],"groups":[]},"assignee":{},"status":""}';

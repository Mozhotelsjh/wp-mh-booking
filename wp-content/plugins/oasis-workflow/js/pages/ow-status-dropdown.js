jQuery( document ).ready( function () {
   ...
   var _post_status_dropdown = function ( selector ) {
      ...
      /**
       * FIXED: if post is published and we update the post then its changing the post status and
       * showing "Publish" button instead of "Update" button
       */
      var selected = ( 'publish' === current_status ) ? 'selected' : '';
      jQuery( selector ).append( jQuery( '<option></option' )
              .attr( 'value', 'publish' )
              .text( 'Published' )
   ...
   if ( jQuery( 'select[name="_status"]' ).length > 0 ) {
      ...
      jQuery( '#the-list a.editinline' ).bind( 'click', function () {
         _post_status_dropdown( '#the-list select[name="_status"]' );
      } );

      // Clean up the bulk edit selector because it's non-standard
      jQuery( '#bulk-edit' ).find( 'select[name="_status"]' ).prepend( '<option value="">hello</option>' );
